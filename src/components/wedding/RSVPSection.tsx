          ...
          <div className="mx-auto max-w-md rounded-3xl border border-lavender-200 bg-white p-10 shadow-lg">
            {submitted === "yes" ? (
              <>
                ...
                <h3 className="font-display text-3xl font-bold text-gray-800">You're In!</h3>
                <p className="mt-3 text-gray-500">We can't wait to celebrate with you. Get ready for an amazing day!</p>
                <p className="mt-2 font-semibold text-lavender-500">We'll save you a seat!</p>
        ...
        >
              ...
              {[
                ...
                >
                  <input
                    ...
                    onChange={(e) => setFormData(p => ({ ...p, attending: e.target.value, events: "" }))}
                    className="sr-only"
                    required
                  />
                  {formData.attending === opt.value && (
          ...
          {formData.attending === "yes" && (
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { value: "both", label: "Ceremony + Dinner", sub: "Morning & evening" },
                  { value: "ceremony", label: "Ceremony only", sub: "Morning only" },
                ].map((opt) => (
                  <label
                    key={opt.value}
                    className={`relative flex cursor-pointer flex-col items-start rounded-2xl border-2 p-4 transition-all ${
                  ...
                  >
                    <input
                      ...
                      onChange={(e) => setFormData(p => ({ ...p, events: e.target.value }))}
                      className="sr-only"
                      required
                    />
                    {formData.events === opt.value && (
